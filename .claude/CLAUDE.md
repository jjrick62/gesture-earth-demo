...
1. **gesture.js 改优先级必须画状态转移表** — 5 种手势 × 5 种过渡 = 25 条路径，逐条验证无冲突后才能提交。禁止孤立改一行优先级
...
10. **改增益/阈值必须对照消费端门槛** — `earth._animate()` 有 `>0.001`/`>0.0001` 守卫，增益值必须让稳态输出跨过这些门槛
11. **新增跨模块共享变量必须明确单一写者** — 用命名约定标识 owner（如 `_gesture*` 系列都是 mapper 写、earth 读）。禁止双写者模式（如之前的 `_pinchRecovery`）
